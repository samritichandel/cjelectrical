<?php
/*
  Plugin Name: Pro WP Limit Login Attempts  
  ...
  Text Domain: pro-wp-limit-login-attempts
  Version: 2.4.9
*/ 
...
function wp_limit_login_update_tables(){
    ...
    add_option( 'limit_login_attepts_captcha', '7', '', 'yes' );
...
function wp_limit_login_init(){
  ...
  function wp_limit_login_head(){ 
      ...
      //pass parameters 
        if(get_option('limit_login_captcha') != 'checked'){
 ...
 function wp_limit_login_failed($username){   
  ...
  if ($_SESSION["popup_flag"] == "true_0152"){
     ...
     if(count($tablerows)==1){
         ...
         $noofattmpt = get_option( 'no_of_wp_limit_login_attepts');
         ...
         $wpdb->insert($tablename,$newdata);
               $remain_attempt = get_option('no_of_wp_limit_login_attepts');  
               $msg = $remain_attempt.' attempts remaining..!';
               return $msg;
     }
    }else{
   ...
   if(count($tablerows)==1){
    ...
    if(($interval->format("%s")) <= 1){
      if(($tablerows[0]->login_attempts)!=0){  
        ...
        $captcha_popup_attempt = get_option( 'limit_login_attepts_captcha');
    ...
    <img src="'.plugin_dir_url( __FILE__ )  .'/images/warn.png""> <b>WP Limit Login attempts Lite</b>
     is a fully functional but limited version of <b><a href="http://ciphercoin.com" target="_blank">WP Limit Login attempts Pro</a></b>. Consider upgrading to get access to all premium features and premium support.
      ...
      if(isset($_POST['attempts'])){
      	if (($_POST['delay']<=4)||($_POST['attempts']<=4)||($_POST['no_captcha']<=1) || ($_POST['no_captcha']>100)) {

      	}else{
      	for ($i=0; $i <= 3 ; $i++) { 
      		if ($i==0) {
      			update_option('no_of_wp_limit_login_attepts',$_POST['attempts']);
      		}
      		if ($i==1) {
      			update_option('limit_login_attepts_delay_time',$_POST['delay']);
      		}
      		if ($i==2) {
      			update_option('limit_login_attepts_captcha',$_POST['no_captcha']);
      		}
          if($i==3){

            if(isset($_POST['enable_captcha']) && $_POST['enable_captcha'] == 'enable'){

               update_option('limit_login_captcha','checked');

            }else{

              update_option('limit_login_captcha','');
            }
     ...
     $limit_no = get_option('no_of_wp_limit_login_attepts');
     $delay_time = get_option('limit_login_attepts_delay_time');
     $captcha_attempts = get_option('limit_login_attepts_captcha');
	echo '<div class="admin_menu">';	
	echo '<h2>WP Limit Login Attempts</h2>';
	echo '<form method="POST" action="" >';
	echo '<div class="row1"><label>Number of login attempts :</label><input type="number" value="'.$limit_no.'" name="attempts" class="attempts" ></div>';
	 if((isset($_POST['attempts'])) && ($_POST['attempts']<=4)){
	 	echo 'Minimum number of allowable login attempts is 5 !!';
	 }
	echo '<div class="row2"><label>Lockdown time in minutes:</label><input type="number" value="'.$delay_time.'"  name="delay" class="delay" ></div>';
	if((isset($_POST['delay'])) && ($_POST['delay']<=4)){
	 	echo 'Minimum number of allowable lockdown time is 5 minutes!!';
	 }
	echo '<div class="row3"><label>Number of attempts for captcha:</label><input type="number" value="'.$captcha_attempts.'" name="no_captcha" class="delay" ></div>';
	if((isset($_POST['no_captcha'])) && ($_POST['no_captcha']==0) && ($_POST['no_captcha']>100)){
	 	echo 'Enter correct number of attempts !!';
	 }
  echo '<div class="row4"><label>Enable captcha:</label>
  <input class="captcha" type="checkbox" name="enable_captcha" value="enable"'. get_option('limit_login_captcha').'></div>';
	...
	/*echo '<div class="warn_msg">
    <img src="'.plugin_dir_url( __FILE__ )  .'/images/warn.png"">   Please consider upgrading to <b><a href="ciphercoin.com/premium/" target="_blank">WP Limit Login attempts Pro</a></b> if you want to use this feature.
  </div>';*/
}
